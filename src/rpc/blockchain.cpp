                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of crown addresses",
...
static RPCHelpMan dumptxoutset()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const fs::path path = fsbridge::AbsPathJoin(GetDataDir(), fs::u8path(request.params[0].get_str()));
    // Write to a temporary path and then move into `path` on completion
    // to avoid confusion due to an interruption.
    const fs::path temppath = fsbridge::AbsPathJoin(GetDataDir(), fs::u8path(request.params[0].get_str() + ".incomplete"));
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/crown/crown/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);

 ...
 *
 * See https://crowntalk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror) {
    ...
    // https://crown.stackexchange.com/a/12556:
...
void PrecomputedTransactionData::Init(const T& txTo, std::vector<CTxOut>&& spent_outputs)
{
    ...
    if (uses_bip143_segwit || uses_bip341_taproot) {
        ...
        m_data_single_hash = GetDataSHA256(txTo);
    }
    if (uses_bip143_segwit) {
        hashPrevouts = SHA256Uint256(m_prevouts_single_hash);
        hashSequence = SHA256Uint256(m_sequences_single_hash);
        hashOutputs = SHA256Uint256(m_outputs_single_hash);
        hashData = SHA256Uint256(m_data_single_hash);
...
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    if (sigversion == SigVersion::WITNESS_V0) {
        ...
        hashData = cacheready ? cache->hashData : SHA256Uint256(GetDataSHA256(txTo));

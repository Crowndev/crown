...
void CTxMemPool::addAddressIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            ...
            CMempoolAddressDelta delta(entry.GetTime(), prevout.nValue * -1, prevout.nAsset, input.prevout.hash, input.prevout.n);
    ...
    for (unsigned int k = 0; k < (tx.nVersion >= TX_ELE_VERSION ? tx.vpout.size() : tx.vout.size()); k++) {
        ...
        if (out.scriptPubKey.IsPayToScriptHash()) {
            ...
            mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetTime(), out.nValue, out.nAsset)));
            inserted.push_back(key);
...
void CTxMemPool::addSpentIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        CSpentIndexValue value = CSpentIndexValue(txhash, j, -1, prevout.nValue, prevout.nAsset, addressType, addressHash);
...
void CTxMemPool::removeRecursive(const CTransaction &origTx, MemPoolRemovalReason reason)
{
    ...
    AssertLockHeld(cs);
        ...
        if (origit != mapTx.end()) {
            ...
            for (unsigned int i = 0; i < (origTx.nVersion >= TX_ELE_VERSION ? origTx.vpout.size() : origTx.vout.size()); i++) {
...
static void CheckInputsAndUpdateCoins(const CTransaction& tx, CCoinsViewCache& mempoolDuplicate, const int64_t spendheight)
{
    ...
    CAmountMap fee_map;
    bool fCheckResult = tx.IsCoinBase() || Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, spendheight, fee_map);
    assert(fCheckResult);

...
const char * const CROWN_CONF_FILENAME = "crown.conf";
const char * const CROWN_SETTINGS_FILENAME = "settings.json";
const char * const CROWN_MASTERNODE_CONF_FILENAME = "masternode.conf";
 ...
 * https://github.com/crown/crown/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //crown-tx using stdin
        std::string val;
...
{
    fs::path settings = GetPathArg("-settings", CROWN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    fs::path pathConfigFile{gArgs.GetPathArg("-mnconf", "systemnode.conf")};
    if (!pathConfigFile.is_absolute()) pathConfigFile = GetDataDir() / pathConfigFile;
    return AbsPathForConfigVal(pathConfigFile, false);
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", CROWN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Crown Core copyright is not removed by accident
    if (copyright_devs.find("Crown Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Crown Core developers";
    }

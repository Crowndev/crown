...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (semMasternodeOutbound == nullptr) {
        // initialize semaphore
        semMasternodeOutbound = std::make_unique<CSemaphore>(MAX_OUTBOUND_MASTERNODE_CONNECTIONS);
    }
...
std::vector<CNode*> CConnman::CopyNodeVector()
{
    ...
    for(size_t i = 0; i < vNodes.size(); ++i) {
        ...
        LogPrint(BCLog::NET, "CConnman::CopyNodeVector -- adding node: peer=%d addr=%s nRefCount=%d fInbound=%d fMasternode=%d\n",
                  pnode->id, pnode->addr.ToString(), pnode->GetRefCount(), pnode->IsInboundConn(), pnode->fMasternode);
        pnode->AddRef();

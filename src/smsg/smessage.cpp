...
// Copyright (c) 202o-2021 The Hemis Core developers
...
void SecMsgBucket::hashBucket(int64_t bucket_time)
{
    ...
    for (auto it = setTokens.begin(); it != setTokens.end(); ++it) {
        ...
        XXH64_update(state, it->sample, 8);
    ...
    uint32_t hash_new = XXH64_digest(state);
...
int CSMSG::AddWalletAddresses()
{
...
#ifdef ENABLE_WALLET
    if (!gArgs.GetBoolArg("-smsgsaddnewkeys", true)) {
    ...
    for (const auto &pw : m_vpwallets) {
        ...
        for (const auto &entry : pw->m_address_book) { // PAIRTYPE(CTxDestination, CAddressBookData)
			...
			provider = pw->GetSolvingProvider(GetScriptForDestination(entry.first));
            CKeyID keyID = GetKeyForDestination(*provider, entry.first);
    ...
    while (fgets(cLine, 512, fp))  {
        ...
        if (strcmp(pName, "key") == 0) {
            ...
            if (rv == 3) {
                ...
                CKeyID k = ToKeyID(std::get<PKHash>(dest));
...
int CSMSG::WriteIni()
{
    ...
    for (std::vector<SecMsgAddress>::iterator it = addresses.begin(); it != addresses.end(); ++it) {
        ...
        CTxDestination dest(PKHash(it->address));
    ...
    for (const auto &tx : block.vtx) {
        ...
        for (size_t i = 0; i < tx->vin.size(); i++) {
            ...
            if (!pubKey.IsValid() || !pubKey.IsCompressed()) {
                LogPrint(BCLog::SMSG, "Public key is invalid %s.\n", HexStr(pubKey));
                continue;
            }
...

        {
            ...
            for (;;) {
                ...
                if (!scan_all && smsg.timestamp + smsg.m_ttl < now) {
                    ...
                    bool fOwnMessage;
                    int rv = ScanMessage(header_buffer, vchData.data(), smsg.nPayload, true, fOwnMessage);
                    if (rv == SMSG_NO_ERROR) {
                        nFoundMessages++;
                    } else {
    ...
    for (fs::directory_iterator itd(pathSmsgDir); itd != itend; ++itd) {
        ...
        bool remove_file = true;
        {
            ...
            for (;;) {
                ...
                // Don't report to gui,
                bool fOwnMessage;
                int rv = ScanMessage(header_buffer, &vchData[0], smsg.nPayload, true, fOwnMessage, true);
                if (rv == 0) {
                    nFoundMessages++;
                } else
...
int CSMSG::GetLocalPublicKey(const std::string &strAddress, std::string &strPublicKey)
{
    ...
    std::unique_ptr<SigningProvider> provider = pactive_wallet->GetSolvingProvider(GetScriptForDestination(dest));
...
int CSMSG::StoreFundingTx(const CTransaction &tx, const CBlockIndex *pindex)
{
    ...
    for (const auto &data : tx.vdata) {
        if (!data->IsVersion(DataTypes::OUTPUT_DATA)) {
            continue;
        }
        const std::vector<uint8_t> &output_data = *data->GetPData();
...
int CSMSG::Validate(const SecureMessage *psmsg, const uint8_t *pPayload, uint32_t nPayload)
{
    ...
    if (psmsg->IsPaidVersion()) {
        ...
        if (ttl < SMSG_MIN_TTL || ttl > SMSG_MAX_PAID_TTL) {
            LogPrint(BCLog::SMSG, "%s , TTL out of range %d.\n", __func__, ttl);
            return SMSG_GENERAL_ERROR;
        }
...
int CSMSG::Encrypt(SecureMessage &smsg, const CKeyID &addressFrom, const CKeyID &addressTo, const std::string &message)
{
    ...
    if (!IsValidDestination(PKHash(addressFrom))) {
        return errorN(SMSG_INVALID_ADDRESS, "%s - addressFrom is not valid: %s.", __func__, EncodeDestination(PKHash(addressFrom)));
    }
    ...
    if (!fSendAnonymous) {
        if (addressFrom.IsNull())
            return errorN(SMSG_INVALID_ADDRESS, "%s - addressFrom is not valid failed: %s.", __func__, EncodeDestination(PKHash(addressFrom)));
    }
...
int CSMSG::Import(SecureMessage *psmsg, std::string &sError, bool setread, bool submitmsg)
{
    ...
    unsigned char header_buffer[SMSG_HDR_LEN];
    psmsg->WriteHeader(header_buffer);
    if (ScanMessage(header_buffer, psmsg->pPayload, psmsg->nPayload, true, fOwnMessage) != 0) {
        // message recipient is not this node (or failed)
        return SMSG_GENERAL_ERROR;
    }
...
int CSMSG::FundMsg(SecureMessage &smsg, std::string &sError, bool fTestFee, CAmount *nFee, size_t *nTxBytes, CCoinControl *coin_control)
{
    ...
    CAmountMap nFeeRet;
    {
        ...
        if (nFee) {
            *nFee = nFeeRet[Params().GetConsensus().subsidy_asset];
        }
        if (nTxBytes) {
...
int CSMSG::Decrypt(bool fTestOnly, const CKey &keyDest, const CKeyID &address, const uint8_t *pHeader, const uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    if (fFromAnonymous) {
        ...
        if (!IsValidDestination(CTxDestination(PKHash(ckidFrom)))) {
        ...
        CTxDestination destr(PKHash(cpkFromSig.GetID()));

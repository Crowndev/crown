...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const CWalletTx::Confirmation& confirm, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/crown/crown/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/crown/crown/pull/13339#issuecomment-461288094
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    if (HaveChain()) {
        std::optional<int> block_height = chain().getBlockHeight(wtx.m_confirm.hashBlock);
        if (block_height) {
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason, uint64_t mempool_sequence) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/crown/crown/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/crown/crown/pull/9371 and the prior sync
        ...
        // https://github.com/crown-core/crown-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
        SyncTransaction(tx, {CWalletTx::Status::UNCONFIRMED, /* block height */ 0, /* block hash */ {}, /* index */ 0});
    }
}
...
isminetype CWallet::IsMine(const CTxOutAsset& txout) const
{
    AssertLockHeld(cs_wallet);
...
CAmountMap CWallet::GetCredit(const CTxOutAsset& txout, const isminefilter& filter) const
{
    CAmountMap val;
        val[txout.nAsset] = txout.nValue;
...
bool CWallet::IsChange(const CTxOutAsset& txout) const
{
...
CAmountMap CWallet::GetChange(const CTxOutAsset& txout) const
{
    AssertLockHeld(cs_wallet);
    CAmountMap change;
        change[txout.nAsset] = txout.nValue;
...
CAmountMap CWallet::GetDebit(const CTransaction& tx, const isminefilter& filter) const
{
    ...
    for (const CTxIn& txin : tx.vin)
    {
        ...
        if (!MoneyRange(nDebit))
            throw std::runtime_error(std::string(__func__) + ": value out of range " + mapToString(nDebit));
    }
    return nDebit;
}
...
bool CWallet::DummySignInput(CMutableTransaction& tx, const size_t nIn, const CTxOutAsset &txout, bool use_max_sig) const
{
...
int CalculateMaximumSignedInputSize(const CTxOutAsset& txout, const CWallet* wallet, bool use_max_sig)
{
                           ...
                           std::list<COutputEntry>& listSent, CAmountMap& mapFee, const isminefilter& filter) const
{
    ...
    for(unsigned int i = 0; i < (tx->nVersion >= TX_ELE_VERSION ? tx->vpout.size() : tx->vout.size()) ; i++){
        ...
        COutputEntry output = {address, txout.nValue, (int)i, txout.nAsset};
 ...
 * @param[in] max_height  std::optional max scanning height. If unset there is
    ...
    while (!fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (chain().findBlock(block_hash, FoundBlock().data(block)) && !block.IsNull()) {
            ...
            if (reorg) {
                ...
                // https://github.com/crown/crown/pull/14711#issuecomment-458342518
...
bool CWallet::IsTrusted(const CWalletTx& wtx, std::set<uint256>& trusted_parents) const
{
    ...
    for (const CTxIn& txin : wtx.tx->vin)
    {
        ...
        const CTxOutAsset& parentOut = (parent->tx->nVersion >= TX_ELE_VERSION ? parent->tx->vpout[txin.prevout.n] : parent->tx->vout[txin.prevout.n]);
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, const CAsset& asset_filter, bool fOnlySafe, const CCoinControl* coinControl, AvailableCoinsType coin_type, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    AssertLockHeld(cs_wallet);
...
std::map<CTxDestination, std::vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    std::vector<COutput> vCoins;
    // include cold
    AvailableCoins(vCoins, CAsset(), true, nullptr, ALL_COINS, 1, MAX_MONEY, 0, 0);
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmountMap& mapTargetValue, std::set<CInputCoin>& setCoinsRet, CAmountMap& mapValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    for (const COutPoint& outpoint : vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
            ...
            CAmount amt = coin.txout.nValue;
            CAsset asset =  coin.txout.nAsset;
            ...
            if (coin_selection_params.use_bnb) {
                ...
                value_to_select[asset] -= coin.txout.nValue;
            }
            mapValueFromPresetInputs[coin.txout.nAsset] += amt;
    ...
    for (std::vector<COutput>::iterator it = vCoins.begin(); it != vCoins.end() && coin_control.HasSelected();) {
        ...
        if (asset != Params().GetConsensus().subsidy_asset && mapTargetValue.find(asset) == mapTargetValue.end()) {
            it = vCoins.erase(it);
        } else {
            ++it;
        }
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmountMap& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    for(unsigned int i = 0; i < (tx.nVersion >= TX_ELE_VERSION ? tx.vpout.size() : tx.vout.size()) ; i++){
        ...
        CRecipient recipient = {txOut.scriptPubKey, txOut.nValue, 0, txOut.nAsset, CAsset(), false, setSubtractFeeFromOutputs.count(i) == 1};
    ...
    if (!CreateTransaction(vecSend, tx_new, nFeeRet, nChangePosInOut, error, coinControl, fee_calc_out, false, CTxDataBase(), ALL_COINS, extraPayloadSize)) {
        return false;
    }
...
OutputType CWallet::TransactionChangeType(const std::optional<OutputType>& change_type, const std::vector<CRecipient>& vecSend)
{
...
bool CWallet::ConvertAsset(CAmountMap &assetin, CTransactionRef& tx, std::string& address, std::string& strFailReason){
    ...
    CAsset asset = assetin.begin()->first;
    CAmount amountin = assetin.begin()->second;
    uint256 thash = uint256();
    bool exists = assetExists(asset, thash);
    if(!exists){
        strFailReason = "Asset not found";
        return false;
    }
    // first check that the asset is convertible
    ...
    CAmount m_amount = rate * amountin;

    std::vector<CRecipient> recipients;

    CRecipient recipient = {GetScriptForDestination(dest) , 0, m_amount, asset , Params().GetConsensus().subsidy_asset, false, true}; // new coins
    recipients.push_back(recipient);
...
bool CWallet::CreateAsset(CAsset& asset, CTransactionRef& tx, std::string& assetname, std::string& shortname, CAmount& inputamt, CAmount& outputamt, int64_t& expiry, int& type, CContract& contract, CTxData& rdata, std::string& strFailReason, bool transferable, bool convertable, bool restricted, bool limited, bool divisible)
{
    ...
    for(auto const& x : AllAssets){
        ...
        if(chain().isequals(x.getShortName() , assetname)){
            strFailReason = "Asset name already reserved as symbol for another asset";
            return false;
        }

        if(chain().isequals(x.getAssetName() , shortname)){
            strFailReason = "Symbol already reserved as another asset's name'";
            return false;
        }

        if(chain().isequals(x.getShortName() , shortname)){
            strFailReason = "Symbol already reserved";
            return false;
        }
    }
    ...
    AssetMetadata meta;
    if (transferable)
        meta.nFlags |= AssetMetadata::AssetFlags::ASSET_TRANSFERABLE;
    if (convertable)
        meta.nFlags |= AssetMetadata::AssetFlags::ASSET_CONVERTABLE;
    if (restricted)
    ...
    if(assetNew.isConvertable() && assetNew.isInflatable()){
       strFailReason = "Asset type is Token, Assets conversion and inflation is currently disabled";
       return false;
    }
    ...
    CRecipient recipient = {GetScriptForDestination(dest), 0, outputamt, Params().GetConsensus().subsidy_asset, assetNew, false, true};
    recipients.push_back(recipient);

    CRecipient burn = {Params().GetConsensus().mandatory_coinbase_destination, inputamt, 0, Params().GetConsensus().subsidy_asset, CAsset(), false, false};
    recipients.push_back(burn);
...
bool CWallet::GetBudgetSystemCollateralTX(CTransactionRef& tx, uint256 hash)
{
    ...
    CAmountMap nFeeRet;
    bilingual_str strFail;
    std::vector<CRecipient> vecSend;
    CRecipient r = {scriptChange, BUDGET_FEE_TX, 0, Params().GetConsensus().subsidy_asset, CAsset(), false, false};
    vecSend.push_back(r);
    int nChangePosRet = -1;
    ...
    bool success = CreateTransaction(vecSend, tx, nFeeRet, nChangePosRet, strFail, coinControl, fee_calc_out, true, CTxDataBase(), ALL_COINS, (CAmount)0);
    if(!success){
        ...
        bool sign, const CTxDataBase& datar, AvailableCoinsType coin_type, int extraPayloadSize)
{
    ...
    for (const auto& recipient : vecSend)
    {
        ...
        if(recipient.fNewAsset || fNewAsset)
            fNewAsset=true;
        ...
        if (recipient.fSubtractFeeFromAmount){
            if(assettosend.nType != 2)
                if(!assettosend.isDivisible())
                    nSubtractFeeFromAmount++;
        }
    }

    if (fUseInstantX && (mapValue[assettosend] > GetSporkValue(SPORK_5_MAX_VALUE) * COIN))
    {
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            AvailableCoins(vAvailableCoins, assettosend, true, &coin_control, ALL_COINS, 1, MAX_MONEY, MAX_MONEY, 0);
            CoinSelectionParams coin_selection_params; // Parameters for coin selection, init with dummy

            if (assettosend != Params().GetConsensus().subsidy_asset){
                //get available CRW to pay the fee
                std::vector<COutput> vAvailableCoinsfee;
            ...
            CTxOutAsset change_prototype_txout(CAsset(), 0, scriptChange);
            ...
            while (true)
            {
                ...
                if (nSubtractFeeFromAmount == 0)
                    mapValueToSelect += nFeeRet;

                if (assettosend != Params().GetConsensus().subsidy_asset)
                    mapValueToSelect[Params().GetConsensus().subsidy_asset] += 0.0001 * COIN;
                ...
                {
                    CTxOutAsset fee(Params().GetConsensus().subsidy_asset, 0.0001 * COIN, CScript());
                    assert(fee.IsFee());
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (recipient.fSubtractFeeFromAmount)
                    {
                        ...
                        if (recipient.asset != assettosend) {
                            error = strprintf(_("Wallet does not support paying fees in different asset, therefore can not subtract fee from address amount, which is of a different asset id. fee asset: %s recipient asset: %s"), assettosend.GetHex(), recipient.asset.GetHex());
                            return false;
                        }
                ...
                if (pick_new_inputs) {
                    ...
                    if (!SelectCoins(vAvailableCoins, mapValueToSelect, setCoins, mapValueIn, coin_control, coin_selection_params, bnb_used))
                    {
                        ...
                        if (bnb_used) {
                            ...
                            LogPrintf("Insufficient funds vAvailableCoins %d\n mapValueToSelect %s\n setCoins %d mapValueIn %s", vAvailableCoins.size(), mapValueToSelect, setCoins.size(), mapValueIn);
                ...
                for(const auto& assetChange : mapChange) {
                    ...
                    else
                    {
                        ...
                        else if ((unsigned int)nChangePosInOut > (txNew.nVersion >= TX_ELE_VERSION ? txNew.vpout.size() : txNew.vout.size()))
                        {
                ...
                std::map<CAsset, int>::const_iterator itPos = vChangePosInOut.find(Params().GetConsensus().subsidy_asset);
                if (itPos != vChangePosInOut.end()) {
                ...
                // Add fee output(s)
                {
                    for(const auto& a : nFeeNeeded) {
                        CTxOutAsset fee(a.first, a.second, CScript());
                        assert(fee.IsFee());
                        if(txNew.nVersion >= TX_ELE_VERSION)
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmountMap fee_needed_with_change {{ assettosend, coin_selection_params.m_effective_feerate.GetFee(tx_size_with_change)}};
                        CAmountMap minimum_value_for_change {{ assettosend,  GetDustThreshold(change_prototype_txout, coin_selection_params.m_discard_feerate)}};
                    ...
                    if (nFeeRet > nFeeNeeded && nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                        CAmountMap extraFeePaid = nFeeRet - nFeeNeeded;
                        std::vector<CTxOut>::iterator change_position = txNew.vout.begin()+nChangePosInOut;
                        std::vector<CTxOutAsset>::iterator change_positionr = txNew.vpout.begin()+nChangePosInOut;
                        if(txNew.nVersion >= TX_ELE_VERSION)
                            change_positionr->nValue += extraFeePaid[change_positionr->nAsset];
                        else
                            change_position->nValue += extraFeePaid[change_positionr->nAsset];
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    CAmountMap additionalFeeNeeded = nFeeNeeded - nFeeRet;
                    std::vector<CTxOut>::iterator change_position = txNew.vout.begin()+nChangePosInOut;
                    std::vector<CTxOutAsset>::iterator change_positionr = txNew.vpout.begin()+nChangePosInOut;

                    if(txNew.nVersion >= TX_ELE_VERSION){
                        // Only reduce change if remaining amount is still a large enough output.
                        if (change_positionr->nValue >= (additionalFeeNeeded + MIN_FINAL_CHANGE)[assettosend]) {
                            change_positionr->nValue -= additionalFeeNeeded[assettosend];
                            nFeeRet += additionalFeeNeeded;
                            for(std::vector<CTxOutAsset>::iterator mi = txNew.vpout.begin(); mi != txNew.vpout.end(); ++mi)
                                if (mi->IsFee() && mi->nAsset == assettosend)
                                    mi->nValue = nFeeRet[assettosend]; // update fee output
                            //txNew.vpout.back().nValue = nFeeRet[assettosend]; // update fee output
                            break; // Done, able to increase fee from change
                        }
                    }
                    else {
                        // Only reduce change if remaining amount is still a large enough output.
                        if (change_position->nValue >= (additionalFeeNeeded + MIN_FINAL_CHANGE)[assettosend]) {
                            change_position->nValue -= additionalFeeNeeded[assettosend];
                            nFeeRet += additionalFeeNeeded;
                            txNew.vout.back().nValue = nFeeRet[assettosend]; // update fee output
                            break; // Done, able to increase fee from change
                        }
        ...
        bool sign, const CTxDataBase& datar, AvailableCoinsType coin_type, int extraPayloadSize)
{
    ...
    bool res = CreateTransactionInternal(vecSend, tx, nFeeRet, nChangePosInOut, error, coin_control, fee_calc_out, sign, datar, coin_type, extraPayloadSize);
    ...
    if (res && nFeeRet > CAmountMap() /* 0 means non-functional fee rate estimation */ && m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        if (CreateTransactionInternal(vecSend, tx2, nFeeRet2, nChangePosInOut2, error2, tmp_cc, fee_calc_out, sign, datar, coin_type, extraPayloadSize)) {
            // if fee of this alternative one is within the range of the max fee, we use this one
            const bool use_aps = nFeeRet2 <= nFeeRet + m_max_aps_fee;
            WalletLogPrintf("Fee non-grouped = %s, grouped = %s, using %s\n", nFeeRet, nFeeRet2, use_aps ? "grouped" : "non-grouped");
            if (use_aps) {
                tx = tx2;
                nFeeRet = nFeeRet2;
...

    {
        ...
        for (const auto& walletEntry : mapWallet)
        {
            ...
            for(unsigned int i = 0; i < (wtx.tx->nVersion >= TX_ELE_VERSION ? wtx.tx->vpout.size() : wtx.tx->vout.size()) ; i++){
                ...
                amount = IsSpent(walletEntry.first, i) ? 0 :  txout.nValue;
 ...
 * https://crowntalk.org/?topic=54527, or
 * https://github.com/crown/crown/pull/1393.
 */
...
std::vector<OutputGroup> CWallet::GroupOutputs(const std::vector<COutput>& outputs, bool single_coin, const size_t max_ancestors) const {
    ...
    for (const auto& output : outputs) {
        if (output.fSpendable) {
            ...
            if (!single_coin && ExtractDestination(mns.scriptPubKey, dst.second)) {

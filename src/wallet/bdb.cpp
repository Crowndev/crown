...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool crown-wallet to salvage or restoring a backup."), fs::quoted(fs::PathToString(file_path)));
            return false;
        }
    }
...
bool ExistsBerkeleyDatabase(const fs::path& path)
{
    ...
    return IsBDBFile(env_directory / fs::PathFromString(data_filename));
}

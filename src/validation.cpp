...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            assert((txFrom->nVersion >= TX_ELE_VERSION ? txFrom->vpout.size() : txFrom->vout.size()) > txin.prevout.n);
            assert((txFrom->nVersion >= TX_ELE_VERSION ? txFrom->vpout[txin.prevout.n] == coin.out : txFrom->vout[txin.prevout.n] == coin.out));
        } else {
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (!m_view.HaveCoin(txin.prevout)) {
            ...
            for (size_t out = 0; out < (tx.nVersion >= TX_ELE_VERSION ? tx.vpout.size() : tx.vout.size()); out++) {
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/crown-dev/2018-November/016518.html
...
CoinsViews::CoinsViews(
    ...
    bool should_wipe) : m_dbview(
                            GetDataDir() / fs::PathFromString(ldb_name), cache_size_bytes, in_memory, should_wipe),
...
bool CScriptCheck::operator()() {
    ...
    if (!VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *txdata), &error))
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (g_txindex) {
            ...
            for (unsigned int k = (tx.nVersion >= TX_ELE_VERSION ? tx.vpout.size() : tx.vout.size()); k-- > 0;) {
                ...
                CAmount nValue;
                CAsset nAsset;
                if (!ExtractIndexInfo(&out, scriptType, hashBytes, nValue, nAsset, pScript)
                    || scriptType == 0) {
                    continue;
                }
...
bool CheckBlockProofPointer(const CBlockIndex* pindex, const CBlock& block, CPubKey& pubkeyMasternode, COutPoint& outpointStakePointer, CTransactionRef& txPayment)
{
    ...
    if (!g_chainman.m_blockman.m_block_index.count(stakePointer.hashBlock))
        return error("%s: Unknown block hash %s", __func__, stakePointer.hashBlock.GetHex());

    CBlockIndex* pindexFrom = g_chainman.m_blockman.m_block_index.at(stakePointer.hashBlock);
    ...
    for (const auto& tx : blockFrom.vtx) {
        if (tx->GetHash() == stakePointer.txid) {
            int rr = (tx->nVersion >= TX_ELE_VERSION ? tx->vpout.size() : tx->vout.size());
            if (rr <= stakePointer.nPos)
...
bool CheckStake(const CBlockIndex* pindex, const CBlock& block, uint256& hashProofOfStake, std::string& errormsg)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (g_txindex) {
            for (unsigned int k = 0; k <  (tx.nVersion >= TX_ELE_VERSION ? tx.vpout.size() : tx.vout.size()) ; k++) {
                ...
                CAmount nValue;
                CAsset nAsset;
                if (!ExtractIndexInfo(&out, scriptType, hashBytes, nValue, nAsset, pScript) || scriptType == 0) {
                    continue;
                }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        if (block.hashMerkleRoot != hashMerkleRoot2){
            LogPrintf("%s\n", block.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_MUTATED, "bad-txnmrklroot", strprintf("hashMerkleRoot mismatch %s vs %s", block.hashMerkleRoot.ToString(), hashMerkleRoot2.ToString()));
        }
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex)
{
    AssertLockNotHeld(cs_main);
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            bool fProofOfStake = nHeight >= chainparams.PoSStartHeight();
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (!fReindex && !fImporting) {
        if (pindex->nHeight >= chainparams.PoSStartHeight()) {
            uint256 hashProofOfStake;
            std::string errormsg;
            if (!CheckStake(pindex, block, hashProofOfStake, errormsg))
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "Proof of stake check failed", strprintf("%s, hashProof %s", errormsg, hashProofOfStake.ToString()));

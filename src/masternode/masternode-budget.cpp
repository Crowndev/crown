...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    CTransactionRef txCollateral = GetTransaction(blockindex, nullptr, nTxCollateralHash, Params().GetConsensus(), nBlockHash);
    ...
    */
    ...
    if (nBlockHash != uint256()) {
        BlockMap::iterator mi = g_chainman.m_blockman.m_block_index.find(nBlockHash);
    ...
    while (it != mapBudgetDrafts.end()) {
        ...
        if(isValid) {
            if(Params().NetworkIDString() == CBaseChainParams::TESTNET || (Params().NetworkIDString() == CBaseChainParams::MAIN && rand() % 4 == 0))
            {
                ...
                pbudgetDraft->AutoCheck(*g_rpc_node->connman);
            } else {
...
{
    assert ((txNew.nVersion >= TX_ELE_VERSION ? txNew.vpout.size() : txNew.vout.size()) == 1); // There is a blank for block creator's reward
...
bool BudgetDraft::AutoCheck(CConnman& connman)
{
    ...
    if (m_voteSubmittedTime && GetTime() - m_voteSubmittedTime.value() < FINAL_BUDGET_VOTE_UPDATE_MIN)
        return false;

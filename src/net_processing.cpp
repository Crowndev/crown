...
void PeerManager::ProcessHeadersMessage(CNode& pfrom, const std::vector<CBlockHeader>& headers, bool via_compact_block)
{
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        if (!LookupBlockIndex(headers[0].hashPrevBlock) && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            m_connman.PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETBLOCKS, ::ChainActive().GetLocator(pindexBestHeader), uint256()));
            LogPrint(BCLog::NET, "received block %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, nUnconnectingHeaders=%d)\n",
...

    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            LogPrint(BCLog::NET, "more getblocks (%d) to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom.GetId(), pfrom.nStartingHeight);
            m_connman.PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETBLOCKS, ::ChainActive().GetLocator(pindexLast), uint256()));
        }
    ...
    while (!orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(m_mempool, state, porphanTx, &removed_txn, false /* bypass_limits */)) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/crown/crown/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/crown/crown/pull/18044#discussion_r443419034
...
/*
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (fRevertToHeaderProcessing) {
            ...
            return ProcessHeadersMessage(pfrom, {cmpctblock.header}, true);
        }
        ...
        if (fBlockReconstructed) {
            ...
            m_chainman.ProcessNewBlock(m_chainparams, pblock, true, &fNewBlock);
            if (fNewBlock) {
...
/*
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        return ProcessHeadersMessage(pfrom, headers, false);
    }
*/
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool inChain = g_chainman.m_blockman.m_block_index.count(pblock->GetHash()) && ::ChainActive().Contains(g_chainman.m_blockman.m_block_index.at(pblock->GetHash()));
        if (pfrom.fSyncingWith && !g_chainman.m_blockman.m_block_index.count(pblock->hashPrevBlock) && pblock->GetHash() == pairHighBlock.second) {
        ...
        if (!inChain && g_chainman.m_blockman.m_block_index.count(pblock->hashPrevBlock) && g_chainman.m_blockman.m_block_index.at(pblock->hashPrevBlock)->nChainTx) {
            bool fNewBlock = false;
    ...
    if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= ::ChainActive().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                m_connman.PushMessage(&pto, msgMaker.Make(NetMsgType::GETBLOCKS, ::ChainActive().GetLocator(state.m_chain_sync.m_work_header->pprev), uint256()));
...
bool PeerManager::SendMessages(CNode* pto)
{
...

    {
        ...
        //
        {
            ...
            std::vector<CBlock> vHeaders;
            bool fRevertToInv =  true; //((!state.fPreferHeaders && (!state.fPreferHeaderAndIDs || pto->vBlockHashesToAnnounce.size() > 1)) || pto->vBlockHashesToAnnounce.size() > MAX_BLOCKS_TO_ANNOUNCE);
        ...
        if (!pto->fClient && ((fFetch && !pto->m_limited_node) || !::ChainstateActive().IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            while (!pto->listAskForBlocks.empty()) {
                ...
                if (!(g_chainman.m_blockman.m_block_index.count(inv.hash) && ::ChainActive().Contains(g_chainman.m_blockman.m_block_index.at(inv.hash)))) {
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    {
        LogPrint(BCLog::NET, "%s: peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED%s\n", __func__, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
        //state->fShouldBan = true;
    } else
        LogPrint(BCLog::NET, "%s: peer=%d (%d -> %d)%s\n", __func__, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
}

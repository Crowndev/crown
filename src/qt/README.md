This directory contains the CrownQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Crown UI.
...
ClientModel is used by the main application `crowngui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a crown:... link from a browser).
...
* UI elements like CrownAmountField, which inherit from QWidget.
* `crownstrings.cpp`: automatically generated
* `crownunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/crown_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "crown-qt" under "Run", which is where you can also add command line arguments)

...
const int CROWN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CROWN_IPC_PREFIX("crown:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(CROWN_IPC_PREFIX, Qt::CaseInsensitive)) // crown: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CROWN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(CROWN_IPC_CONNECT_TIMEOUT);
...
//
// OSX-specific way of handling crown: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("crown://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'crown://' is not a valid URI. Use 'crown:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(CROWN_IPC_PREFIX, Qt::CaseInsensitive)) // crown: URI
    {

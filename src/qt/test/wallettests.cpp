...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<CrownAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_crown-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_crown-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_crown-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = CrownUnits::formatWithUnit(unit, balance, false, CrownUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    CrownAmountField* amountInput = receiveCoinsDialog.findChild<CrownAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_crown-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);

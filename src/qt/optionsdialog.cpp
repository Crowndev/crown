    ...
    mapper(nullptr)
{
    ...
    ui->crownAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->crownAtStartup);
    ...
    ui->crownAtStartup->setToolTip(ui->crownAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->crownAtStartup->setText(ui->crownAtStartup->text().arg(PACKAGE_NAME));

    ui->openCrownConfButton->setToolTip(ui->openCrownConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new CrownUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->crownAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openCrownConfButton_clicked()
{

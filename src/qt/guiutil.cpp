...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new CrownAddressEntryValidator(parent));
...
QString formatCrownURI(const SendAssetsRecipient &info)
{
    ...
    QString ret = QString("crown:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Crown (%s)\n", chain);

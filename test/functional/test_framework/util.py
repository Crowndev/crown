...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `CrownTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain):
    ...
    with open(os.path.join(datadir, "crown.conf"), 'w', encoding='utf8') as f:
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "crown.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "crown.conf")):
        with open(os.path.join(datadir, "crown.conf"), 'r', encoding='utf8') as f:
            for line in f:

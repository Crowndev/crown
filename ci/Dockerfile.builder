...
RUN apt-get update && apt-get install $APT_ARGS python python3-pip python3-setuptools && rm -rf /var/lib/apt/lists/*
...
# crown
# RUN git clone https://a889564d387b6a527768dad98179ae8c5f91a53d@github.com/TheExiledMonk/Crown.git
...
RUN useradd -u ${USER_ID} -g crown -s /bin/bash -m -d /crown crown
...
RUN apt-get update && apt-get install $APT_ARGS python python3-zmq && rm -rf /var/lib/apt/lists/*
...
RUN mkdir /crown-src && \
  ...
  chown $USER_ID:$GROUP_ID /crown-src && \

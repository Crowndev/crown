...
export BUILDER_IMAGE_NAME="crown-builder-$BUILD_TARGET-$JOB_NUMBER"
...
export MAKEJOBS="-j$(cat /proc/cpuinfo | awk '/^processor/{print $3}' | tail -n 1)"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export CROWN_CONFIG="--enable-glibc-back-compat --enable-reduce-exports CXXFLAGS=-Wno-psabi"
elif [ "$BUILD_TARGET" = "win32" ]; then
  export HOST=i686-w64-mingw32
  export DPKG_ADD_ARCH="i386"
  export CROWN_CONFIG="--enable-gui --enable-reduce-exports --disable-miner"
  ...
  export CROWN_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-stacktraces LDFLAGS=-static-libstdc++"
  ...
  export CROWN_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-stacktraces"
  ...
  export CROWN_CONFIG="--enable-glibc-back-compat --enable-reduce-exports"
  export RUN_UNITTESTS=true
elif [ "$BUILD_TARGET" = "linux64_release" ]; then
  export HOST=x86_64-pc-linux-gnu
  export DEP_OPTS="NO_UPNP=1"
  export CROWN_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  ...
  export ANDROID_TOOLCHAIN_BIN=/opt/android_ndk/toolchains/llvm/prebuilt/linux-x86_64/bin 
  export ANDROID_SDK=/opt/android-sdk-linux 
  export ANDROID_NDK=/opt/android_ndk
  export CONFIG_SITE=$PWD/depends/aarch64-linux-android/share/config.site

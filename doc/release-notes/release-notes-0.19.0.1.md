Crown Core version 0.19.0.1 is now available from:
...
Crown Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Crown Core on unsupported systems.
...
to open the Crown Core .dmg. This is due to new signing requirements
imposed by Apple, which the Crown Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Crown Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Crown
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Crown Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Crown Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Crown Core. (#14054)

- To eliminate well-known denial-of-service vectors in Crown Core,
  ...
  This prevents Crown Core from sending the BIP111 NODE_BLOOM service
  ...
  of Crown Core and nodes that have manually enabled BIP37 support,
...
- By default, Crown Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Crown Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Crown Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Crown Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Crown address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Crown Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Crown AEAD (jonasschnelli)
...
- #15773 Add CrownTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Crown Core (luke-jr)

...
Crown Core version 0.21.0 is now available from:
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Crown Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Crown Core will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when Crown Core has finished with its startup
  sequence. (#15367)
...
Crown Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Crown Core will now just log warnings instead of creating new wallets with
...
Users who already have a Crown Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Crown Core.
...
The change to using descriptors changes the default derivation paths used by Crown Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Crown Core is
...
- #18739 Document how to fuzz Crown Core using Honggfuzz (practicalswift)

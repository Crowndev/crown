Crown Core version 0.18.0 is now available from:
...
`/Applications/Crown-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
recommended to use Crown Core on unsupported systems.
...
10.10. Additionally, Crown Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Crown Core
...
`~bitcoin/.bitcoin`. This change makes Crown Core more consistent with
...
- A new document introduces Crown Core's BIP174 [Partially-Signed
  Crown Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Crown-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Crown Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Crown Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Crown Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside Crown
...
This section describes planned changes to Crown Core that may affect
other Crown software and services.

- Since version 0.16.0, Crown Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Crown Core 0.20 (expected about
  a year after 0.18), Crown Core will default to native segwit
  ...
  to bech32 addresses, and if the Crown Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Crown Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Crown Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Crown Core
...
- A sub-project of Crown Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Crown Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Crown Core's own implementation, although entropy
  gathered by Crown Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Crown Core a
...
- On macOS, Crown Core now opts out of application CPU throttling
...
- #11625 Add CrownApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for Crown Core (sipsorcery)
...
- #12775 Integration of property based testing into Crown Core (Christewart)

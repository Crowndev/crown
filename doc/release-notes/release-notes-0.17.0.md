Crown Core version 0.17.0 is now available from:
...
BIP 174 Partially Signed Crown Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Crown transactions that are not fully signed
...
Overall, the construction of a fully signed Crown transaction goes through the
following steps:
...
- An **Extractor** produces a valid Crown transaction (in network format)
...
- #13528 `b877c39` Move CrownGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use CrownTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use CrownTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use CrownTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use CrownTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for CrownTestFramework (WillAyd)

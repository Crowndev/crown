...
Crown Core version 0.19.1 is now available from:
...
From Crown Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Crown Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Crown Core does not yet change appearance when

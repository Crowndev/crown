...
Crown Core version 0.20.0 is now available from:
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Crown
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Crown Core on
unsupported systems.

From Crown Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
...
#### Removal of BIP61 reject network messages from Crown Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Crown Core version 0.18.0.
...
- Testing or debugging of implementations of the Crown P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Crown Core. Crown Core logs debug messages
...
- OpenSSL is no longer used by Crown Core.  (#17265)

- BIP70 support has been fully removed from Crown Core. The
...
- All custom settings configured when Crown Core starts are now
...
- The "Start Crown Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Crown Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Crown Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of Crown Core should support a GUI option
...
- Crown Core's automatically-generated source code documentation is
...
- #16983 Add detailed info about Crown Core files (hebasto)

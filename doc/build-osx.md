...
This guide describes how to build crownd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Crown Core from source.
...
### 4. Clone Crown repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Crown Core repository to a directory.
...
It is not necessary to build wallet functionality to run `crownd` or  `crown-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/crown/crown/blob/master/doc/descriptors.md), `sqlite` is required.
...
Crown Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/crown/crown/issues/7714).
...
You can deploy a `.dmg` containing the Crown Core application using `make deploy`.
...
There are many ways to configure Crown Core, here are a few common examples:
...
Run the following in your terminal to compile Crown Core:
...
Crown Core should now be available at `./src/crownd`.
If you compiled support for the GUI, it should be available at `./src/qt/crown-qt`.

The first time you run `crownd` or `crown-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Crown/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Crown"

touch "/Users/${USER}/Library/Application Support/Crown/crown.conf"
...
```shell
./src/crownd -daemon      # Starts the crown daemon.
./src/crown-cli --help    # Outputs a list of command-line options.
./src/crown-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/crown-qt -server # Starts the crown-qt server mode, allows crown-cli control
```

...
The Crown Core daemon can be configured to act as a trusted "border
router", implementing the crown wire protocol and relay, making
...
The ZeroMQ feature in Crown Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building crownd:
...
For instance:
    ...
    $ crownd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/crownd.tx.raw \
...
These options can also be provided in crown.conf.
...
From the perspective of crownd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into crownd directly. Furthermore, no information is
...
using. Crownd appends an up-counting sequence number to each

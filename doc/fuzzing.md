# Fuzzing Crown Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Crown Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/crown/crown
$ cd crown/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/crown/crown/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/crown/crown/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/crown/crown/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/crown/crown/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`crown-core/qa-assets`](https://github.com/crown-core/qa-assets) repo.

To fuzz `process_message` using the [`crown-core/qa-assets`](https://github.com/crown-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/crown-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`crown-core/qa-assets`](https://github.com/crown-core/qa-assets) repo.

Every single pull request submitted against the Crown Core repo is automatically tested against all inputs in the [`crown-core/qa-assets`](https://github.com/crown-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Crown Core more robust.
...
with certain assembly code from Crown Core's code. See [developer notes on sanitizers](https://github.com/crown/crown/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Crown Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing Crown Core using [`afl-fuzz`](https://github.com/google/afl):
...
# Fuzzing Crown Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Crown Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Crown Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Crown
Core without having to write any custom fuzzing harness. The `crownd` server
...
```sh
$ mkdir crown-honggfuzz-p2p/
$ cd crown-honggfuzz-p2p/
$ git clone https://github.com/crown/crown
$ cd crown/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/crownd.cpp b/src/crownd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/crownd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
